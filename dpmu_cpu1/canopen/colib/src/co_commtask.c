...
*-------------------------------------------------------------------
* $Id: co_commtask.c 41218 2022-07-12 10:35:19Z boe $
*
*
*-------------------------------------------------------------------
*
...
* The CANopen Stack of emotas is a software library
* that provides all communication services
* of the "CANopen Application Layer and Communication Profile"
* CiA 301 V4.2,
* "CANopen-FD Application Layer and Communication Profile"
* CiA 301 V5.0,
* and other profiles of CiA e.V. and EN50325-4
*
*
...
# include "ico_esdoserver.h"
#  ifdef CO_ESDO_CLIENT_SUPPORTED
#   include "ico_esdoclient.h"
#  endif /* CO_ESDO_CLIENT_SUPPORTED */
...
#ifdef RAWCAN_SUPPORTED
# include "ico_rawcan.h"
#endif /* RAWCAN_SUPPORTED */
#ifdef J1939_SDG_SUPPORTED
# include "ij1939_sdg.h"
#endif /* J1939_SDG_SUPPORTED */
...
#define COMM_STATE_TABLE_CNT	13u
...
---------------------------------------------------------------------------*/
static BOOL_T	eventFlagMsg;
static BOOL_T	eventFlagTimer;
static BOOL_T	eventFlagEventHandler;
static CO_CAN_STATE_T canState;
static CO_COMM_STATE_EVENT_T recQuState;
static CO_COMM_STATE_EVENT_T trQuState;
#ifdef CO_EVENT_CAN_CNT
static UNSIGNED8	canStateTableCnt;
static UNSIGNED8	commEventTableCnt;
...
* \return BOOL_T
* \retval CO_TRUE
* 	further actions necessary - call coCommTask again
* \retval CO_FALSE
* 	all actions passed
*/
BOOL_T coCommTask(
...
/***************************************************************************/
/**
* \brief coCommTaskCheck - check communication tasks
*
* This function checks if all communication task are done.
* If not, it returns true. In this case, coCommTask() should be called again.
*
* \return BOOL_T
* \retval CO_TRUE
* 	further actions necessary - call coCommTask again
...
*
*/
BOOL_T coCommTaskCheck(
		void	/* no parameter */
	)
{
	/* check for messages at receive buffer */
	if (coQueueReceiveMessageAvailable() == CO_TRUE)  {
		return(CO_TRUE);
	}

	/* check for events */
	if (icoEventsAvailable() == CO_TRUE)  {
		return(CO_TRUE);
	}
...
# endif /* CO_SDO_NETWORKING */
		...
		case CO_SERVICE_LSS_S_RECEIVE:
			icoLssHandler(&recData);
			break;
#endif /* CO_LSS_SUPPORTED */

#ifdef CO_LSS_MASTER_SUPPORTED
		case CO_SERVICE_LSS_M_RECEIVE:
			icoLssMasterHandler(&recData);
			break;
#endif /* CO_LSS_MASTER_SUPPORTED */
...
#ifdef CO_ESDO_SUPPORTED
		case CO_SERVICE_ESDO_SERVER:
			icoEsdoServerHandler(&recData);
			break;
#endif /* CO_ESDO_SUPPORTED */

#ifdef CO_ESDO_CLIENT_SUPPORTED
		case CO_SERVICE_ESDO_CLIENT:
			icoEsdoClientHandler(&recData);
			break;
#endif /* CO_ESDO_CLIENT_SUPPORTED */

#ifdef ISOTP_CLIENT_CONNECTION_CNT
		case CO_SERVICE_ISOTP_CLIENT:
			iisoTpClientMessageHandler(&recData);
			break;
#endif /* ISOTP_CLIENT_CONNECTION_CNT */

#ifdef ISOTP_SERVER_CONNECTION_CNT
		case CO_SERVICE_ISOTP_SERVER:
# ifdef ISOTP_CLIENT_CONNECTION_CNT
			iisoTpClientMessageHandler(&recData);
# endif /* ISOTP_CLIENT_CONNECTION_CNT */
			iisoTpServerMessageHandler(&recData);
			break;
#endif /* ISOTP_SERVER_CONNECTION_CNT */

#ifdef CO_IDNEGO_SUPPORTED
		case CO_SERVICE_IDNEGO_RECEIVE:
			icoIdNegoHandler(&recData);
			break;
#endif /* CO_IDNEGO_SUPPORTED */

#ifdef RAWCAN_SUPPORTED
		case CO_SERVICE_RAWCAN_RECEIVE:
			irawcanReceive(&recData);
			break;
#endif /* CAN_RAW_SUPPORTED */
...
{
typedef enum {
	CST_CAN, CST_TRQ, CST_RECQ, CST_DRV
} COMM_STATE_TYPE_T;
...
static const COMM_STATE_TABLE_T	commTable[COMM_STATE_TABLE_CNT] = {
	...
	{ CO_COMM_STATE_EVENT_DRV_ERROR,		CST_DRV,	CO_CAN_STATE_UNCHANGED}
};
UNSIGNED8	i;
...
UNSIGNED8	cnt;
	...
	/* check for generic driver event */
	if (commTable[i].type == CST_DRV)  {
		newState = newEvent;
	}
...
{
	/* set new indication function as first at the list */
	if (commEventTableCnt >= CO_EVENT_CAN_CNT) {
		return(RET_EVENT_NO_RESSOURCE);
	}
