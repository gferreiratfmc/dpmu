...
*-------------------------------------------------------------------
* $Id: co_emcy.c 40827 2022-05-30 12:59:49Z boe $
*
*
*-------------------------------------------------------------------
*
...
static RET_T classicalEmergency(UNSIGNED16 emcyErrCode,
		CO_CONST UNSIGNED8	pData[]);
#endif /* CO_EMCY_PRODUCER */
#ifdef CO_EMCY_CONSUMER_CNT
static CO_EMCY_CONSUMER_T *getEmcyConsumer(UNSIGNED8 idx);
#endif /* CO_EMCY_CONSUMER_CNT */
...
static UNSIGNED8		emcyTableCnt;
# endif /* CO_EVENT_EMCY_CNT */
#endif /* CO_EMCY_PRODUCER */

#ifdef CO_EMCY_CONSUMER_CNT
static UNSIGNED8	emcyConsumerCnt;
...
static UNSIGNED8		emcyConsTableCnt;
# endif /* CO_EVENT_EMCY_CONSUMER_CNT */
#ifdef CO_CAN_TIMESTAMP_SUPPORTED
static CO_CAN_TIMESTAMP_T	emcyTimeStamp;
...
RET_T	retVal;
	...
	retVal = classicalEmergency(emcyErrCode, pData);
	
	return(retVal);
}


static RET_T classicalEmergency(
		UNSIGNED16	emcyErrCode,		/* emergency error code */
		CO_CONST UNSIGNED8	pData[]		/* pointer to additional 5 data bytes */
	)
{
...
UNSIGNED8	*pErrorRegister;
	...
	pErrorRegister = (UNSIGNED8 *)coOdGetObjAddr(0x1001u, 0u);
	...
	trData[0] = (UNSIGNED8)(emcyErrCode & 0xffu);
	trData[1] = (UNSIGNED8)(emcyErrCode >> 8u);
	if (pErrorRegister != NULL)  {
		trData[2] = *pErrorRegister;
	} else {
		trData[2] = 0u;
	}
	...
	/* transmit data */
	retVal = icoTransmitMessage(emcyCob, &trData[0], 0u);
...
#ifdef CO_EVENT_STATIC_EMCY
	retVal = coEventEmcyInd(emcyErrCode, pAddErrorBytes);
	if (retVal != RET_OK)  {
		return(RET_INTERNAL_ERROR);
	}
#endif /* CO_EVENT_STATIC_EMCY */
...
UNSIGNED8	emcyBuf[5];
UNSIGNED32	mask;
CO_EMCY_CONSUMER_T	*pEmcyCons;
# endif /* CO_EMCY_NETWORKING */
# ifdef CO_EVENT_EMCY_CONSUMER_CNT
UNSIGNED8	cnt;
# endif /* CO_EVENT_EMCY_CONSUMER_CNT */
UNSIGNED8	msgCnt = CO_EMCY_MSG_CNT;


	/* check for correct message len */
	if (pRecData->msg.len != msgCnt)  {
		return;
	}
...
# ifdef CO_CAN_TIMESTAMP_SUPPORTED
	...
	emcyTimeStamp = pRecData->msg.timestamp;
# endif /* CO_CAN_TIMESTAMP_SUPPORTED */
...
# ifdef CO_EVENT_EMCY_CONSUMER_CNT
	...
	while (cnt > 0u)  {
		cnt--;
		/* call user indication */
		emcyConsTable[cnt]((UNSIGNED8)(pRecData->msg.canId & 0x7fu),
			pRecData->msg.data[0]
...
# ifdef CO_EVENT_STATIC_EMCY_CONSUMER
	coEventEmcyConsumerInd((UNSIGNED8)(pRecData->msg.canId & 0x7fu),
			pRecData->msg.data[0]
				| (UNSIGNED16)((UNSIGNED16)pRecData->msg.data[1] << 8u),
			pRecData->msg.data[2],
			&pRecData->msg.data[3]);
...
# ifdef CO_EMCY_NETWORKING
	pEmcyCons = getEmcyConsumer((UNSIGNED8)pRecData->spec);
	mask = pEmcyCons->routingList;
	if (mask != 0u) {
		{
			if ((mask & 1ul) != 0u)  {
				/* forward emcy */
				...
				emcyBuf[2] = (UNSIGNED8)(pRecData->msg.canId & 0x7fu);
...
/***************************************************************************/
/**
* \brief coEmcyGetTimestamp - get EMCY timestamp
*
* This function returns the timestamp of the last received Emergency
* set by the CAN driver
*
* \return 
*
*/
CO_CAN_TIMESTAMP_T coEmcyGetTimestamp(
		void	/* no parameter */
	)
{
	return(emcyTimeStamp);
}
# endif /* CO_CAN_TIMESTAMP_SUPPORTED */
...
#if defined(CO_EMCY_CONSUMER_CNT) || defined(CO_EMCY_NETWORKING)
	pEmcyCons = getEmcyConsumer(subIndex - 1u);
#endif /* defined(CO_EMCY_CONSUMER_CNT) || defined(CO_EMCY_NETWORKING) */
...
# endif /* CO_EMCY_ERROR_HISTORY */
		...
		case 0x1028u:	/* emcy consumer */
			pAddr = (void *)&pEmcyCons->cobId;
			break;
#endif /* CO_EMCY_CONSUMER_CNT */

#ifdef CO_EMCY_NETWORKING
		case 0x1f2fu:	/* emcy routing list */
			pAddr = (void *)&pEmcyCons->routingList;
			break;
# endif /* CO_EMCY_NETWORKING */
...
(void)subIndex;
	...
	if (index == 0x1028u)  {
		if ((canId & CO_COB_VALID_MASK) == 0u)  {
			/* new cobid is valid, only allowed if cob was disabled before*/
			pEmcyCons = getEmcyConsumer(subIndex - 1u);
			if ((pEmcyCons->cobId & CO_COB_VALID_MASK)
					!= CO_COB_INVALID)  {
				return(RET_SDO_INVALID_VALUE);
			}
			if (icoCheckRestrictedCobs(canId, 0x81u, 0x81u) == CO_TRUE)  {
...
# endif /* CO_EMCY_ERROR_HISTORY */
		...
		case 0x1014u:
			retVal = icoCobSet(emcyCob, emcyCobId,
				CO_COB_RTR_NONE, cnt);
			break;
		...
		case 0x1028u:	/* emcy consumer */
			...
			pEmcyCons = getEmcyConsumer(subIndex - 1u);
			(void)icoCobSet(pEmcyCons->cob,
					pEmcyCons->cobId, CO_COB_RTR_NONE, cnt);
			break;
#endif /* CO_EMCY_CONSUMER_CNT */
		default:
			break;
...
UNSIGNED8 cnt = CO_EMCY_MSG_CNT;
	...
	(void)icoCobSet(emcyCob, emcyCobId,
		CO_COB_RTR_NONE, cnt);
}
...
void icoEmcyVarInit(
		...
		UNSIGNED8	*pEmcyActErrHistList,	/* line counts */
...
* \internal
*
* \brief getEmcyConsumer - get emcy consumer
*
* \return none
*/
static CO_EMCY_CONSUMER_T *getEmcyConsumer(
		UNSIGNED8		idx
	)
{
	return(&emcyCons[idx]);
...
CO_EMCY_CONSUMER_T	*pEmcyCons;
	...
	for (i = 0u; i < emcyConsumerCnt; i++)  {
		pEmcyCons = getEmcyConsumer(i);


		(void)icoCobSet(pEmcyCons->cob, pEmcyCons->cobId,
			CO_COB_RTR_NONE, cnt);
	}
}
...
CO_EMCY_CONSUMER_T	*pEmcyCons;
	...
	for (i = 0u; i < emcyConsumerCnt; i++)  {
		pEmcyCons = getEmcyConsumer(i);
		retVal = coOdGetDefaultVal_u32(0x1028u, i + 1u, &pEmcyCons->cobId);
		if (retVal != RET_OK)  {
			pEmcyCons->cobId |= CO_COB_INVALID;
		}

#ifdef CO_EMCY_NETWORKING
		(void)coOdGetDefaultVal_u32(0x1f2fu, i + 1u,
			&pEmcyCons->routingList);
#endif /* CO_EMCY_NETWORKING */
	}
}
...
CO_EMCY_CONSUMER_T	*pEmcyCons;
	...
	for (cnt = 0u; cnt < emcyCnt; cnt++)  {
		pEmcyCons = getEmcyConsumer(cnt);
		pEmcyCons->cob = icoCobCreate(CO_COB_TYPE_RECEIVE,
				CO_SERVICE_EMCY_RECEIVE, (UNSIGNED16)cnt);
		if (pEmcyCons->cob == 0xffffu)  {
			return(RET_NO_COB_AVAILABLE);
		}
	}
