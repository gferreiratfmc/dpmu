...
*-------------------------------------------------------------------
* $Id: co_odaccess.c 41921 2022-09-01 10:39:03Z boe $
*
*
*-------------------------------------------------------------------
*
...
static CO_CONST CO_OD_ASSIGN_T		*pOdAssignTable;
...
static CO_CONST UNSIGNED32		*pOdConst_VS_MaxLen = NULL;
static VIS_STRING		*pOdPtr_VS = NULL;
static CO_CONST UNSIGNED32		*pOdPtr_VS_MaxLen = NULL;
static UNSIGNED32		*pOdPtr_VS_ActLen = NULL;
static OCTET_STRING		* CO_CONST *pOdPtr_OS = NULL;
static CO_CONST UNSIGNED32		*pOdPtr_OS_MaxLen =  NULL;
static UNSIGNED32		*pOdPtr_OS_ActLen =  NULL;
static CO_DOMAIN_PTR 	*pOdPtr_Domain = NULL; /*lint !e961 pointer to generic domains */
static UNSIGNED32		*pOdPtr_Domain_MaxLen = NULL;
static UNSIGNED32		*pOdPtr_Domain_ActLen = NULL;
#ifdef CO_EVENT_STORE
static BOOL_T	loadParameterState = { CO_FALSE };
#endif /* CO_EVENT_STORE */
#ifdef CO_STORE_SUPPORTED
static OD_LAST_STORE_T	odLastStore;
#endif /* CO_STORE_SUPPORTED */
...
/**
*
* \brief coOdGetObjAddr - get address of a writable object
*
...
UNSIGNED8	*pAddr;
	...
	pAddr = odGetObjAddr(pDesc, subIndex);
...
/**
*
* \brief coOdGetObjAddrR - get address of a readable object
*
* Get the address of an object from the object dictionary
* for read access
*
...
CO_CONST CO_OBJECT_DESC_T *pDesc;
RET_T	retVal;
CO_CONST UNSIGNED8	*pAddr;

	retVal = coOdGetObjDescPtr(index, subIndex, &pDesc);
	if (retVal != RET_OK)  {
		return(NULL);
	}
	pAddr = odGetObjAddr_R(pDesc, subIndex);
...
RET_T	retVal;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U8_NMT:
			retVal = icoOdGetObj(pDesc, subIndex, pObj, 0u, 0u, CO_TRUE);
			break;
...
#ifdef CO_BOOTLOADER_MODE
#else /* CO_BOOTLOADER_MODE */
/***************************************************************************/
/**
...
BOOL_T	changed = CO_FALSE;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U8_VAR:
		case CO_DTYPE_U8_PTR:
			retVal = icoOdPutObj(pDesc, subIndex, &newVal, 0u, 0u, CO_TRUE, &changed);
			break;
	...
	if (retVal == RET_OK)  {
		retVal = icoEventObjectChanged(pDesc, index, subIndex, changed);
	}
...
BOOL_T	changed = CO_FALSE;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U32_STORE:
			...
			if (retVal == RET_OK)  {
				retVal = icoOdPutObj(pDesc, subIndex, &newVal, 0u, 0u, CO_TRUE,
					&changed);
			}
			break;
...
UNSIGNED16	attr = pDesc->attr;
	...
	pObjAddr = odGetObjAddr_R(pDesc, subIndex);
	if (pObjAddr != NULL)  {
		/* get object size */
...
#endif /* CO_EXTENDED_DATA_TYPES */
		default:
			pObjAddr = odGetObjAddr(pDesc, subIndex);
			break;
	}

	if (pObjAddr != NULL)  {
		size = coOdGetObjMaxSize(pDesc);
...
UNSIGNED16	offs;
	...
	/* offset depends on attribute CO_ATTR_COMPACT */
	if ((pDesc->attr & CO_ATTR_COMPACT) != 0u)  {
		offs = (pDesc->tableIdx + subIndex) - pDesc->subIndex;
	} else {
		offs = pDesc->tableIdx;
	}
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U8_CONST:
			if (pOdConst_u8 != NULL)  {
				pObjAddr = &pOdConst_u8[offs];
			}
			break;
		case CO_DTYPE_U16_CONST:
			if (pOdConst_u16 != NULL)  {
				pObjAddr = &pOdConst_u16[offs];
			}
			break;
		case CO_DTYPE_U32_CONST:
			/* table initialized ? */
			if (pOdConst_u32 != NULL)  {
				pObjAddr = &pOdConst_u32[offs];
			}
			break;
		case CO_DTYPE_I8_CONST:
			/* table initialized ? */
			if (pOdConst_i8 != NULL)  {
				pObjAddr = &pOdConst_i8[offs];
			}
			break;
		case CO_DTYPE_I16_CONST:
			/* table initialized ? */
			if (pOdConst_i16 != NULL)  {
				pObjAddr = &pOdConst_i16[offs];
			}
			break;
		case CO_DTYPE_I32_CONST:
			/* table initialized ? */
			if (pOdConst_i32 != NULL)  {
				pObjAddr = &pOdConst_i32[offs];
			}
			break;
		case CO_DTYPE_R32_CONST:
			/* table initialized ? */
			if (pOdConst_r32 != NULL)  {
				pObjAddr = &pOdConst_r32[offs];
			}
			break;
		...
		case CO_DTYPE_U24_CONST:
			if (pOdConst_u24 != NULL)  {
				pObjAddr = &pOdConst_u24[offs];
			}
			break;
		case CO_DTYPE_U40_CONST:
			if (pOdConst_u40 != NULL)  {
				pObjAddr = &pOdConst_u40[offs];
			}
			break;
		case CO_DTYPE_U48_CONST:
			if (pOdConst_u48 != NULL)  {
				pObjAddr = &pOdConst_u48[offs];
			}
			break;
		case CO_DTYPE_U64_CONST:
			if (pOdConst_u64 != NULL)  {
				pObjAddr = &pOdConst_u64[offs];
			}
			break;
#endif /* CO_EXTENDED_DATA_TYPES */
...
UNSIGNED16	offs;
	...
	if ((pDesc->attr & CO_ATTR_DYNOD) != 0u)  {
		pObjAddr = coDynOdGetObjAddr(pDesc);
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U8_VAR:
			/* table initialized ? */
			if (pOdVar_u8 != NULL)  {
				pObjAddr = &pOdVar_u8[offs];
			}
			break;
		case CO_DTYPE_U16_VAR:
			/* table initialized ? */
			if (pOdVar_u16 != NULL)  {
				pObjAddr = &pOdVar_u16[offs];
			}
			break;
		case CO_DTYPE_U32_VAR:
			/* table initialized ? */
			if (pOdVar_u32 != NULL)  {
				pObjAddr = &pOdVar_u32[offs];
			}
			break;
		case CO_DTYPE_R32_VAR:
			/* table initialized ? */
			if (pOdVar_r32 != NULL)  {
				pObjAddr = &pOdVar_r32[offs];
			}
			break;
		case CO_DTYPE_U8_PTR:
		case CO_DTYPE_BOOL_PTR:
			if (pOdPtr_u8 != NULL)  {
				pObjAddr = pOdPtr_u8[offs];
			}
			break;
		case CO_DTYPE_U16_PTR:
			if (pOdPtr_u16 != NULL)  {
				pObjAddr = pOdPtr_u16[offs];
			}
			break;
		case CO_DTYPE_U32_PTR:
			/* table initialized ? */
			if (pOdPtr_u32 != NULL)  {
				pObjAddr = pOdPtr_u32[offs];
			}
			break;
		case CO_DTYPE_I8_VAR:
			/* table initialized ? */
			if (pOdVar_i8 != NULL)  {
				pObjAddr = &pOdVar_i8[offs];
			}
			break;
		case CO_DTYPE_I16_VAR:
			/* table initialized ? */
			if (pOdVar_i16 != NULL)  {
				pObjAddr = &pOdVar_i16[offs];
			}
			break;
		case CO_DTYPE_I32_VAR:
			/* table initialized ? */
			if (pOdVar_i32 != NULL)  {
				pObjAddr = &pOdVar_i32[offs];
			}
			break;
		case CO_DTYPE_I8_PTR:
			if (pOdPtr_i8 != NULL)  {
				pObjAddr = pOdPtr_i8[offs];
			}
			break;
		case CO_DTYPE_I16_PTR:
			if (pOdPtr_i16 != NULL)  {
				pObjAddr = pOdPtr_i16[offs];
			}
			break;
		case CO_DTYPE_I32_PTR:
			/* table initialized ? */
			if (pOdPtr_i32 != NULL)  {
				pObjAddr = pOdPtr_i32[offs];
			}
			break;
		case CO_DTYPE_R32_PTR:
			/* table initialized ? */
			if (pOdPtr_r32 != NULL)  {
				pObjAddr = pOdPtr_r32[offs];
			}
			break;
		case CO_DTYPE_VS_CONST:
			if (pOdConst_VS != NULL)  {
				pObjAddr = pOdConst_VS[offs];
			}
			break;
		case CO_DTYPE_VS_PTR:
			if (pOdPtr_VS != NULL)  {
				pObjAddr = pOdPtr_VS[offs];
			}
			break;
		case CO_DTYPE_OS_PTR:
			if (pOdPtr_OS != NULL)  {
				pObjAddr = pOdPtr_OS[offs];
			}
			break;
		case CO_DTYPE_DOMAIN:
			if (pOdPtr_Domain != NULL)  {
				pObjAddr = pOdPtr_Domain[offs];
			}
			break;
#ifdef CO_SDO_SERVER_CNT
		case CO_DTYPE_U8_SDO_SERVER:
		...
		case CO_DTYPE_U16_NMT:
		case CO_DTYPE_U32_NMT:
			pObjAddr = icoNmtGetObjectAddr(pDesc->tableIdx, subIndex);
			break;
#endif /* defined(CO_NMT_MASTER) || defined(CO_SELF_STARTING_DEVICE) */
		...
		case CO_DTYPE_U24_VAR:
			/* table initialized ? */
			if (pOdVar_u24 != NULL)  {
				pObjAddr = &pOdVar_u24[offs];
			}
			break;
		case CO_DTYPE_U40_VAR:
			/* table initialized ? */
			if (pOdVar_u40 != NULL)  {
				pObjAddr = &pOdVar_u40[offs];
			}
			break;
		case CO_DTYPE_U48_VAR:
			/* table initialized ? */
			if (pOdVar_u48 != NULL)  {
				pObjAddr = &pOdVar_u48[offs];
			}
			break;
		case CO_DTYPE_U64_VAR:
			/* table initialized ? */
			if (pOdVar_u64 != NULL)  {
				pObjAddr = &pOdVar_u64[offs];
			}
			break;

		case CO_DTYPE_U24_PTR:
			if (pOdPtr_u24 != NULL)  {
				pObjAddr = pOdPtr_u24[offs];
			}
			break;
		case CO_DTYPE_U40_PTR:
			if (pOdPtr_u40 != NULL)  {
				pObjAddr = pOdPtr_u40[offs];
			}
			break;
		case CO_DTYPE_U48_PTR:
			if (pOdPtr_u48 != NULL)  {
				pObjAddr = pOdPtr_u48[offs];
			}
			break;
		case CO_DTYPE_U64_PTR:
			if (pOdPtr_u64 != NULL)  {
				pObjAddr = pOdPtr_u64[offs];
			}
			break;
#endif /* CO_EXTENDED_DATA_TYPES */
...
RET_T	retVal;
	...
	pOdPtr_Domain[pDesc->tableIdx] = pAddr;
	pOdPtr_Domain_MaxLen[pDesc->tableIdx] = size;
	pOdPtr_Domain_ActLen[pDesc->tableIdx] = size;
...
*
* This function change the address and current length if a visible string object.
* It can only be used for non-constant strings,
* defined as user variable.
* Please note, the maximal object len will not be changed.
* 
* \return RET_T
*
*/
...
RET_T	retVal;
	...
	pOdPtr_VS[pDesc->tableIdx] = pAddr;
	pOdPtr_VS_ActLen[pDesc->tableIdx] = size;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U8_PTR:
			...
			if ((pDesc->attr & CO_ATTR_LIMIT) != 0u)  {
				if (pOdConst_u8 != NULL)  {
					if ((value >= pOdConst_u8[pDesc->limitMinIdx]) 
					 && (value <= pOdConst_u8[pDesc->limitMaxIdx]))  {
						retVal = RET_OK;
					}
				}
			} else
...
(void)pDesc;
	...
	if ((pDesc->attr & CO_ATTR_LIMIT) != 0u)  {
		if (pOdConst_u16 != NULL)  {
			if ((value >= pOdConst_u16[pDesc->limitMinIdx]) 
			 && (value <= pOdConst_u16[pDesc->limitMaxIdx]))  {
				retVal = RET_OK;
			}
		}
	} else
...
RET_T	retVal = RET_INVALID_PARAMETER;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U32_PTR:
			...
			if ((pDesc->attr & CO_ATTR_LIMIT) != 0u)  {
				if (pOdConst_u32 != NULL)  {
					if ((value >= pOdConst_u32[pDesc->limitMinIdx]) 
					 && (value <= pOdConst_u32[pDesc->limitMaxIdx]))  {
						retVal = RET_OK;
					}
				}
			} else
...
(void)pData;
	...
	switch (dType)  {
		...
		case CO_DTYPE_R32_PTR:
			if (pOdConst_r32 != NULL)  {
				(void)coNumMemcpyPack(&r32, pData, 4u, 1u, 0u);
...
/**
*
* \brief coOdSetObjSize - set object size
*
* This function set the size of an object (in bytes),
* given by the object description.
*
* Only valid for strings and domains
*
*
*/
RET_T coOdSetObjSize(
		CO_CONST CO_OBJECT_DESC_T	*pDesc,	/**< pointer for description index */
		UNSIGNED32					size	/**< new object size */
	)
{
#ifdef CO_DYNAMIC_OBJDIC
	if ((pDesc->attr & CO_ATTR_DYNOD) != 0u)  {
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_OS_PTR:
			if (pOdPtr_OS_ActLen != NULL)  {
				pOdPtr_OS_ActLen[pDesc->tableIdx] = size;
			}
			break;
		case CO_DTYPE_DOMAIN:
...
/***************************************************************************/
/**
* \brief coOdGetObjMaxSize - get object maximum size
*
* This function returns the maximum size of an object (in bytes),
* given by the object description.
*
* The maximum size is used for strings and domains and cannot be changed.
*
* \return maximum object size
*
*/
UNSIGNED32 coOdGetObjMaxSize(
		CO_CONST CO_OBJECT_DESC_T	*pDesc	/**< pointer for description index */
	)
{
UNSIGNED32	size = 0u;
	...
	switch (pDesc->dType)  {
		case CO_DTYPE_VS_PTR:
			if (pOdPtr_VS_MaxLen != NULL)  {
				size = pOdPtr_VS_MaxLen[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_OS_PTR:
			if (pOdPtr_OS_MaxLen != NULL)  {
				size = pOdPtr_OS_MaxLen[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_DOMAIN:
			if (pOdPtr_Domain_MaxLen != NULL)  {
				size = pOdPtr_Domain_MaxLen[pDesc->tableIdx];
			}
			break;
		default:
			size = coOdGetObjSize(pDesc);
...
#if defined(CO_PDO_RECEIVE_CNT) || defined(CO_SRDO_SUPPORTED)
/***************************************************************************/
/**
* \internal
*
...
RET_T icoOdGetObjRecMapData(
		...
		UNSIGNED8       bitLen,     /* bit length in mapping entry */
...
UNSIGNED16	attr;
	...
	*pVar = odGetObjAddr(pDesc, subIndex);
...
#endif /* defined(CO_PDO_RECEIVE_CNT) || defined(CO_SRDO_SUPPORTED) */


#if defined(CO_PDO_TRANSMIT_CNT) || defined(CO_SRDO_SUPPORTED)
/***************************************************************************/
/**
* \internal
*
...
UNSIGNED16	attr;
	...
	*pVar = odGetObjAddr_R(pDesc, subIndex);
...
#endif /* defined(CO_PDO_TRANSMIT_CNT) || defined(CO_SRDO_SUPPORTED) */
...
CO_CONST CO_OBJECT_DESC_T *pDesc;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U8_NETWORK:
		case CO_DTYPE_U8_SRDO:
			*pDefVal = pOdConst_u8[pDesc->defValIdx];
			break;
		default:
...
CO_CONST CO_OBJECT_DESC_T *pDesc;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U16_SRDO:
		case CO_DTYPE_U16_FLYMA:
			*pDefVal = pOdConst_u16[pDesc->defValIdx];
			break;
		default:
			retVal = RET_SDO_DATA_TYPE_NOT_MATCH;
	}
...
CO_CONST CO_OBJECT_DESC_T *pDesc;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_U32_SRDO:
		case CO_DTYPE_U32_STORE:
			*pDefVal = pOdConst_u32[pDesc->defValIdx];
			break;
		default:
			retVal = RET_SDO_DATA_TYPE_NOT_MATCH;
	}
...
/***************************************************************************/
/**
* \brief coOdGetDefaultVal_r32 - get default value for specific object 
*
* This function returns the default value of an REAL32 object.
*
* \return RET_T
*
*/
RET_T coOdGetDefaultVal_r32(
		UNSIGNED16		index,			/**< index */
		UNSIGNED8		subIndex,		/**< sub index */
		REAL32			*pDefVal		/**< pointer to default val */
	)
{
RET_T	retVal;
CO_CONST CO_OBJECT_DESC_T *pDesc;
	...
	switch (pDesc->dType)  {
		...
		case CO_DTYPE_R32_PTR:
		case CO_DTYPE_R32_CONST:
			*pDefVal = pOdConst_r32[pDesc->defValIdx];
			break;
		default:
			retVal = RET_SDO_DATA_TYPE_NOT_MATCH;
	}
...
RET_T	retVal;
	...
	if (retVal != RET_OK)  {
#ifdef CO_DYNAMIC_OBJDIC
		retVal = coDynOdGetObjDescPtr(index, subIndex, pDescPtr);
		if (retVal == RET_IDX_NOT_FOUND)  {
		UNSIGNED8 nbrSubs;
			nbrSubs = icoOdGetNumberOfSubs(index);
			if (nbrSubs != 0u)  {
...
UNSIGNED16	descIdx;
UNSIGNED16	nrOfSubs;
UNSIGNED16	highestSub;

	/* reset all pointer */
	odLastStore.assign = 0u;
	odLastStore.subIdx = 0u;
	odLastStore.idx = 0u;

	/* for all OD objects */
	for (odIdx = 0u; odIdx < odAssignCnt; odIdx++) {
		nrOfSubs = pOdAssignTable[odIdx].numberOfSubs;
		highestSub = pOdAssignTable[odIdx].highestSub;
		descIdx = pOdAssignTable[odIdx].odDescIdx;

		/* for all subs of this object */
		for (sub = 0u; sub < nrOfSubs; sub++)  {
			/* get attribute */
			attr = pObjDescTable[descIdx + sub].attr;
			/* compact desc subindex */
			if ((attr & CO_ATTR_COMPACT) == CO_ATTR_COMPACT) {
				if ((attr & CO_ATTR_STORE) == CO_ATTR_STORE) {
					UNSIGNED16 tmp = (UNSIGNED16)(highestSub - sub) + 1u;
					count += tmp;
				}
			} else {
				if ((attr & CO_ATTR_STORE) == CO_ATTR_STORE) {
...
CO_CONST CO_OBJECT_DESC_T * pDesc = NULL;
	...
	/* for faster access, check start address */
	/* is exact the same since last call */
	if (odLastStore.idx == idx) {
		*pIndex = odLastStore.odIndex;
		*pSubIndex = odLastStore.odSubIndex;
		return(RET_OK);
	}

	if (idx < odLastStore.idx) {
		odLastStore.assign = 0u;
		odLastStore.idx = 0u;
		odLastStore.subIdx = 0u;
	}

	for (u16 = odLastStore.assign; u16 < odAssignCnt; u16++) {

		numberOfSubs = pOdAssignTable[u16].numberOfSubs;
		odDescIdx = pOdAssignTable[u16].odDescIdx;
		highestSub = pOdAssignTable[u16].highestSub;

		for (u8 = odLastStore.subIdx; u8 <= highestSub; u8++) {
			if (u8 >= numberOfSubs)  {
				pDesc = &pObjDescTable[(odDescIdx + numberOfSubs) - 1u];
				odLastStore.odSubIndex = u8;
			} else {
				pDesc = &pObjDescTable[odDescIdx + u8];
				odLastStore.odSubIndex = pDesc->subIndex;
			}
			/* compact desc subindex */
			if ((pDesc->attr & CO_ATTR_STORE) == CO_ATTR_STORE) {
				...
				if (odLastStore.idx == idx) {
					odLastStore.idx = idx;
					odLastStore.assign = u16;
					odLastStore.subIdx = u8 + 1u;
					odLastStore.odIndex = pOdAssignTable[u16].index;
					*pIndex = odLastStore.odIndex;
					*pSubIndex = odLastStore.odSubIndex;
...
RET_T		retVal;
	...
	while (left <= right)  {
		/* l + r / 2 */
		middle = (left + right) >> 1;
		if (pOdAssignTable[middle].index == index)  {
			mainIndex = middle;
			found = CO_TRUE;
	...
	left = pOdAssignTable[middle].odDescIdx;
	right = (left + (UNSIGNED16)pOdAssignTable[middle].numberOfSubs) - 1u;
	found = CO_FALSE;
	...
	while (left <= right)  {
		/* l + r / 2 */
		middle = (left + right) >> 1;
		if (pObjDescTable[middle].subIndex == subIndex)  {
			found = CO_TRUE;
			breakLoop = CO_TRUE;	/* break; */
		}
		if (pObjDescTable[middle].subIndex > subIndex)  {
	...
	if (found == CO_FALSE) {
		retVal = RET_SUBIDX_NOT_FOUND;
        if (pOdAssignTable[mainIndex].highestSub >= subIndex)  {
			middle = pOdAssignTable[mainIndex].odDescIdx;
			middle += (UNSIGNED16)pOdAssignTable[mainIndex].numberOfSubs - 1u;
			if((pObjDescTable[middle].attr & CO_ATTR_COMPACT) == CO_ATTR_COMPACT)  {
				*pDescIdx = middle;
				retVal = RET_OK;
			}
		}
...
CO_CONST CO_OBJECT_DESC_T *pDesc;
	...
	if (subIndex == 255u)  {
		if (index != 0xffffu)  {
			...
			/* if all object don't search the object */
			retVal = RET_OK;
		}
	} else {
		retVal = odGetObjDescIdx(index, subIndex, &idx);
	...
	if (retVal != RET_OK)  {
		if (subIndex == 255u)  {
			retVal = coDynOdGetObjDescPtr(index, 0u, &pDesc);
		} else {
			retVal = coDynOdGetObjDescPtr(index, subIndex, &pDesc);
		}
	}
#endif /* CO_DYNAMIC_OBJDIC */	
	if (retVal != RET_OK)  {
...
UNSIGNED16	idx = 0u;
	...
	while (idx < objectEventTableCnt)  {
		if ((objectEventTable[idx].index == index)
		 || (objectEventTable[idx].index == 0xffffu))  {
...
RET_T		retVal = RET_OK;
	...
	switch (pDesc->dType)  {
#ifdef CO_SDO_SERVER_CNT
		case CO_DTYPE_U32_SDO_SERVER:
			retVal = icoSdoObjChanged(pDesc->tableIdx, subIndex);
			break;
#endif /* CO_SDO_SERVER_CNT */

#ifdef CO_SDO_CLIENT_CNT
		case CO_DTYPE_U32_SDO_CLIENT:
			retVal = icoSdoClientObjChanged(pDesc->tableIdx, subIndex);
			break;
#endif /* CO_SDO_CLIENT_CNT */
		...
		case CO_DTYPE_U16_TPDO:
		case CO_DTYPE_U32_TPDO:
			retVal = icoTrPdoObjChanged(pDesc->tableIdx, subIndex);
			break;
...
# endif /* CO_TR_PDO_DYN_MAP_ENTRIES */
		...
		case CO_DTYPE_U16_RPDO:
		case CO_DTYPE_U32_RPDO:
			retVal = icoRecPdoObjChanged(pDesc->tableIdx, subIndex);
			break;
...
# endif /* CO_REC_PDO_DYN_MAP_ENTRIES */
		...
		case CO_DTYPE_U16_ERRCTRL:
		case CO_DTYPE_U32_ERRCTRL:
			retVal = icoErrCtrlObjChanged(pDesc->tableIdx, subIndex);
			break;
#endif /* CANOPEN_SUPPORTED */
		...
		case CO_DTYPE_U16_EMCY:
#ifdef CO_EMCY_PRODUCER
			retVal = icoEmcyObjChanged(pDesc->tableIdx, subIndex);
#endif /* CO_EMCY_PRODUCER */
			break;
		case CO_DTYPE_U32_EMCY:
#if defined(CO_EMCY_PRODUCER) || defined (CO_EMCY_CONSUMER_CNT)
...
#ifdef CO_STORE_SUPPORTED
		case CO_DTYPE_U32_STORE:
			retVal = icoStoreObjChanged(pDesc->tableIdx, subIndex);
			break;
#endif /* CO_STORE_SUPPORTED */
		...
		case CO_DTYPE_U16_NMT:
		case CO_DTYPE_U32_NMT:
			retVal = icoNmtObjChanged(pDesc->tableIdx, subIndex);
			break;
#endif /* defined(CO_NMT_MASTER) || defined(CO_SELF_STARTING_DEVICE) */
		...
		case CO_DTYPE_U16_SRDO:
		case CO_DTYPE_U32_SRDO:
			retVal = icoSrdoObjChanged(pDesc->tableIdx, subIndex, pDesc->dType);
			break;
#endif /* CO_SRDO_SUPPORTED */
...
UNSIGNED16	offs;
	...
	for (i = 0u; i < odAssignCnt; i++)  {
		if (pOdAssignTable[i].index == index)  {
			offs = pOdAssignTable[i].odDescIdx;
			offs += pOdAssignTable[i].numberOfSubs;
			offs -= 1u;
			if ((pObjDescTable[offs].attr & CO_ATTR_COMPACT) != 0u ) {
				return(pOdAssignTable[i].highestSub + 1u);
			} else {
				return(pOdAssignTable[i].numberOfSubs);
			}
		}
	}
...
* \internal
*
* \brief icoOdSetLoadParaState - sets new load state internally
*
*
* \return none
*
...
REAL32		*p_r32;
	...
	/* start at index */
	for (i = 0u; i < odAssignCnt; i++)  {
		if ((pOdAssignTable[i].index >= startIdx)
		 && (pOdAssignTable[i].index <= lastIdx))  {
			/* reset all subs */
			for (sub = 0u; sub < pOdAssignTable[i].numberOfSubs; sub++)  {
				pDesc = &pObjDescTable[pOdAssignTable[i].odDescIdx + sub];
				...
				if ((pDesc->attr & CO_ATTR_DEFVAL) != 0u)  {
					switch (pDesc->dType)  {
						...
						case CO_DTYPE_BOOL_VAR:
							CO_OS_LOCK_OD
							pOdVar_u8[pDesc->tableIdx] = pOdConst_u8[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_I8_VAR:
							CO_OS_LOCK_OD
							pOdVar_i8[pDesc->tableIdx] = pOdConst_i8[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U16_VAR:
							CO_OS_LOCK_OD
							pOdVar_u16[pDesc->tableIdx] = pOdConst_u16[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_I16_VAR:
							CO_OS_LOCK_OD
							pOdVar_i16[pDesc->tableIdx] = pOdConst_i16[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U32_VAR:
							CO_OS_LOCK_OD
							pOdVar_u32[pDesc->tableIdx] = pOdConst_u32[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_R32_VAR:
							CO_OS_LOCK_OD
							pOdVar_r32[pDesc->tableIdx] = pOdConst_r32[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_I32_VAR:
							CO_OS_LOCK_OD
							pOdVar_i32[pDesc->tableIdx] = pOdConst_i32[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U8_PTR:
						case CO_DTYPE_BOOL_PTR:
							p_u8 = pOdPtr_u8[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_u8 = pOdConst_u8[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_I8_PTR:
							p_i8 = pOdPtr_i8[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_i8 = pOdConst_i8[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U16_PTR:
							p_u16 = pOdPtr_u16[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_u16 = pOdConst_u16[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_I16_PTR:
							p_i16 = pOdPtr_i16[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_i16 = pOdConst_i16[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U32_PTR:
							p_u32 = pOdPtr_u32[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_u32 = pOdConst_u32[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_I32_PTR:
							p_i32 = pOdPtr_i32[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_i32 = pOdConst_i32[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_R32_PTR:
							p_r32 = pOdPtr_r32[pDesc->tableIdx];
							CO_OS_LOCK_OD
							*p_r32 = pOdConst_r32[pDesc->defValIdx];
							CO_OS_UNLOCK_OD
							break;
						...
						case CO_DTYPE_U24_VAR:
							CO_OS_LOCK_OD
							memcpy(&pOdVar_u24[pDesc->tableIdx],
								&pOdConst_u24[pDesc->defValIdx], 3u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U24_PTR:
							CO_OS_LOCK_OD
							memcpy(pOdPtr_u24[pDesc->tableIdx],
								&pOdConst_u24[pDesc->defValIdx], 3u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U40_VAR:
							CO_OS_LOCK_OD
							memcpy(&pOdVar_u40[pDesc->tableIdx],
								&pOdConst_u40[pDesc->defValIdx], 5u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U40_PTR:
							CO_OS_LOCK_OD
							memcpy(pOdPtr_u40[pDesc->tableIdx],
								&pOdConst_u40[pDesc->defValIdx], 5u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U48_VAR:
							CO_OS_LOCK_OD
							memcpy(&pOdVar_u48[pDesc->tableIdx],
								&pOdConst_u48[pDesc->defValIdx], 6u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U48_PTR:
							CO_OS_LOCK_OD
							memcpy(pOdPtr_u48[pDesc->tableIdx],
								&pOdConst_u48[pDesc->defValIdx], 6u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U64_VAR:
							CO_OS_LOCK_OD
							memcpy(&pOdVar_u64[pDesc->tableIdx],
								&pOdConst_u64[pDesc->defValIdx], 8u);
							CO_OS_UNLOCK_OD
							break;
						case CO_DTYPE_U64_PTR:
							CO_OS_LOCK_OD
							memcpy(pOdPtr_u64[pDesc->tableIdx],
								&pOdConst_u64[pDesc->defValIdx], 8u);
							CO_OS_UNLOCK_OD
							break;
#endif /* CO_EXTENDED_DATA_TYPES */
	...
	)
{
	...
	pOdPtr_r32 = NULL;
	pOdConst_VS = NULL;
	pOdConst_VS_MaxLen = NULL;
	pOdPtr_VS = NULL;
	pOdPtr_VS_MaxLen = NULL;
	pOdPtr_VS_ActLen = NULL;
	pOdPtr_OS = NULL;
	pOdPtr_OS_MaxLen =  NULL;
	pOdPtr_OS_ActLen =  NULL;
	pOdPtr_Domain = NULL;
	pOdPtr_Domain_MaxLen = NULL;
	pOdPtr_Domain_ActLen = NULL;
...
*/
void coOdInitOdPtr(
		const CO_OD_ASSIGN_T	*pOdAssign,	/**< pointer to OD assign */
...
(void)pEventPtr;
	...
	pOdConst_VS_MaxLen = pOdVarPointers->odConstVisStringLen;
	pOdPtr_VS = pOdVarPointers->odVisString;
	pOdPtr_VS_MaxLen = pOdVarPointers->odVisStringLen;
	pOdPtr_VS_ActLen = pOdVarPointers->odVisStringActLen;

	pOdPtr_OS = pOdVarPointers->odOctetString;
	pOdPtr_OS_MaxLen = pOdVarPointers->odOctetStringLen;
	pOdPtr_OS_ActLen = pOdVarPointers->odOctetStringActLen;
	pOdPtr_Domain = pOdVarPointers->domainPtr;
	pOdPtr_Domain_MaxLen = pOdVarPointers->domainLen;
	pOdPtr_Domain_ActLen = pOdVarPointers->domainActLen;
