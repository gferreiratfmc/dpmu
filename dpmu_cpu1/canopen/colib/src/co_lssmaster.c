...
*-------------------------------------------------------------------
* $Id: co_lssmaster.c 41218 2022-07-12 10:35:19Z boe $
*
*
*-------------------------------------------------------------------
*
...
---------------------------------------------------------------------------*/
static RET_T lssStartCommand(const UNSIGNED8	*pData,
...
{
UNSIGNED8 *pData = &pRecData->msg.data[0];
	...
	)
{
	...
	(void)coNumMemcpyPack(&inquireData, pData, 4u, CO_ATTR_NUM, 0u);
...
static UNSIGNED16	timeOut;
	...
	if (interval != 0u)  {
		timeOut = timeOutVal;
		retVal = coTimerStart(&lssNonConfigSlavesTimer,
			interval * 1000000ul, nonConfiguredSlaves, &timeOut,
...
RET_T	retVal;
	...
	trData[0] = LSS_CS_IDENT_VENDOR;
	memcpy(&trData[1], &vendorId, 4);
	lssCmd = 0;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	}

	trData[0] = LSS_CS_IDENT_PRODUCT;
	memcpy(&trData[1], &productCode, 4);
	lssCmd = 0;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	...
	trData[0] = LSS_CS_IDENT_REVISION_LOW;
	memcpy(&trData[1], &revisionLow, 4);
	lssCmd = 0;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	...
	trData[0] = LSS_CS_IDENT_REVISION_HIGH;
	memcpy(&trData[1], &revisionHigh, 4);
	lssCmd = 0;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	...
	trData[0] = LSS_CS_IDENT_SERIAL_LOW;
	memcpy(&trData[1], &serialNumberLow, 4);
	lssCmd = 0;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	...
	trData[0] = LSS_CS_IDENT_SERIAL_HIGH;
	memcpy(&trData[1], &serialNumberHigh, 4);
	lssCmd = 0;
	retVal = lssStartCommand(&trData[0], lssMasterTimeout, timeOutVal);
	if (retVal != RET_OK)  {
		return(retVal);
	}
...
RET_T	retVal;
	...
	memset(&trData[0], 0u, CO_CAN_MAX_DATA_LEN);
	trData[0] = LSS_CS_FAST_SCAN;
	trData[5] = 128u;
	retVal = lssStartCommand(&trData[0], lssMasterFastScanTimeout, timeOutVal);
	if (retVal != RET_OK)  {
		return(retVal);
	}
...
RET_T	retVal;
	...
	memset(&trData[0], 0u, CO_CAN_MAX_DATA_LEN);
	trData[0] = LSS_CS_ACTIVATE_BITRATE;
	trData[1] = switchDelay & 0xffu;
	trData[2] = switchDelay >> 8u;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	lssCmd = 0u;
	if (retVal != RET_OK)  {
		return(retVal);
	...
	retVal = coTimerStart(&lssBitrateTimer,
			switchDelay * 1000ul, bitrateSwitchFct, NULL,
			CO_TIMER_ATTR_ROUNDUP_CYCLIC); /*lint !e960 */
		/* Derogation MisraC2004 R.16.9 function identifier used without '&'
		 * or parenthesized parameter */
...
RET_T	retVal;
	...
	memset(&trData[0], 0u, CO_CAN_MAX_DATA_LEN);
	trData[0] = LSS_CS_SWITCH_GLOBAL;
	trData[1] = (UNSIGNED8)mode;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	lssCmd = 0u;
...
RET_T	retVal;
	...
	/* vendor id */
	trData[0] = LSS_CS_SWITCH_SEL_VENDOR;
	memcpy(&trData[1], &vendorId, 4u);
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	}
	...
	/* product code */
	trData[0] = LSS_CS_SWITCH_SEL_PRODUCT;
	memcpy(&trData[1], &productCode, 4u);
	lssCmd = 0u;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	...
	/* revision */
	trData[0] = LSS_CS_SWITCH_SEL_REVISION;
	memcpy(&trData[1], &versionNr, 4u);
	lssCmd = 0u;
	retVal = lssStartCommand(&trData[0], NULL, 0u);
	if (retVal != RET_OK)  {
		return(retVal);
	...
	/* serialNr */
	trData[0] = LSS_CS_SWITCH_SEL_SERIAL;
	memcpy(&trData[1], &serNr, 4u);
	lssCmd = 0u;
	retVal = lssStartCommand(&trData[0], lssMasterTimeout, timeOutVal);
	if (retVal != RET_OK)  {
		return(retVal);
...
RET_T	retVal;
	...
	memset(&trData[0], 0u, CO_CAN_MAX_DATA_LEN);
	trData[0] = LSS_CS_INQUIRE_NODEID;
	retVal = lssStartCommand(&trData[0], lssMasterTimeout, timeOutVal);
...
RET_T	retVal;
	...
	switch (subIndex)  {
		case 1u:
			trData[0] = LSS_CS_INQUIRE_VENDOR;
			break;
		case 2u:
			trData[0] = LSS_CS_INQUIRE_PRODUCT;
			break;
		case 3u:
			trData[0] = LSS_CS_INQUIRE_REVISION;
			break;
		case 4u:
			trData[0] = LSS_CS_INQUIRE_SERIAL;
			break;
...
RET_T	retVal;
	...
	if (timeOutVal != 0u)  {
		retVal = coTimerStart(&lssTimer, timeOutVal * 1000ul,
			timeOutFct, NULL, CO_TIMER_ATTR_ROUNDUP); /*lint !e960 */
		/* Derogation MisraC2004 R.16.9 function identifier used without '&'
		 * or parenthesized parameter */
	}
...
static const SERV_CS_T	servCs[] = {
	...
	{ LSS_CS_IDENTITY_SLAVE,	CO_LSS_MASTER_SERVICE_IDENTITY}
};
...
UNSIGNED8	trData[CO_CAN_MAX_DATA_LEN];	/* data */
	...
	trData[0] = LSS_CS_FAST_SCAN;
	trData[1] = pFastScan->idNr & 0xffu;
	trData[2] = (pFastScan->idNr >> 8u) & 0xffu;
	trData[3] = (pFastScan->idNr >> 16u) & 0xffu;
	trData[4] = (pFastScan->idNr >> 24u) & 0xffu;
	trData[6] = lssSub;
	trData[7] = lssSub; 
	...
	trData[5] = pFastScan->bitCheck;
	(void)lssStartCommand(&trData[0], lssMasterFastScanTimeout,
		fsTimeOut);
	identifyBitVal = 1u;
}
...
RET_T	retVal;
	...
	memset(&trData[0], 0u, CO_CAN_MAX_DATA_LEN);
	trData[0] = LSS_CS_NON_CONFIG_REMOTE_SLAVE;
	retVal = lssStartCommand(&trData[0], lssMasterTimeout, timeOutVal);
...
# ifdef CO_EVENT_LSS_CNT
	...
	while (cnt > 0u)  {
		...
		if ((service == CO_LSS_MASTER_SERVICE_FASTSCAN) 
			|| (service == CO_LSS_MASTER_SERVICE_INQUIRE_ADDRESS))  {
...
* \internal
*
* \brief icoLssFastScanCmd - send a single lss fastscan command
*
* \return RET_T
*
*/
RET_T icoLssFastScanCmd(
		UNSIGNED32	lIdNumber,
		UNSIGNED8	lBitCheck,
		UNSIGNED8	lLssSub,
		UNSIGNED8	lLssNext,
		UNSIGNED16	lTimeOutVal			/* timeout value in msec */
	)
{
UNSIGNED8	trData[CO_CAN_MAX_DATA_LEN];	/* data */
RET_T	retVal;
	...
	memcpy(&trData[1], &lIdNumber, 4u);

	trData[5] = lBitCheck;
	trData[6] = lLssSub;
	trData[7] = lLssNext;
	retVal = lssStartCommand(&trData[0], lssMasterFastScanTimeout, lTimeOutVal);
	if (retVal != RET_OK)  {
		return(retVal);
	}
...
{
	lssTransCob = icoCobCreate(CO_COB_TYPE_TRANSMIT, CO_SERVICE_LSS_M_TRANSMIT, LSS_CAN_ID_TYPE_CC);
	if (lssTransCob == 0xffffu)  {
		return(RET_NO_COB_AVAILABLE);
	}

	lssRecCob = icoCobCreate(CO_COB_TYPE_RECEIVE, CO_SERVICE_LSS_M_RECEIVE, LSS_CAN_ID_TYPE_CC);
