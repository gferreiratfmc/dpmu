...
*-------------------------------------------------------------------
* $Id: co_nmt.c 41921 2022-09-01 10:39:03Z boe $
*
*
*-------------------------------------------------------------------
*
...
UNSIGNED8			cnt;
	...
	/* ignore message if we don't have a node id */
...
# endif /* CO_NMT_MASTER */
	...
	/* disable all cobs */
	icoCobDisableAll(CO_SERVICE_CANOPEN_START, CO_SERVICE_CANOPEN_END);
...
# ifdef CO_FLYING_MASTER_SUPPORTED
	icoFlymaSetDefaultValue();
# endif /* # ifdef CO_FLYING_MASTER_SUPPORTED */
...
{
	/* for not CANopen lines return always PRE-OP */
	if ((icoProtocolType() & MP_PROT_TYPE_CANOPEN) == 0u)  {
		return(CO_NMT_STATE_PREOP);
	}
	...
	)
{
	...
	{
		nmtCob = 0xffffu;
		persistantNodeId = *pNodeId;
		nodeIdFunction = *nodeIdFct;
		nodeId = 127u;
		nmtState = CO_NMT_STATE_RESET_NODE;
	}
