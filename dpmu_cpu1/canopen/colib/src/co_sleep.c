...
*-------------------------------------------------------------------
* $Id: co_sleep.c 33185 2020-08-21 13:23:38Z boe $
*
*
*-------------------------------------------------------------------
*
...
#endif /* CO_SLEEP_447 */
static SLEEP_STATE_T	sleepState = { SLEEP_STATE_AWAKE };
...
UNSIGNED8	trData[CO_CAN_MAX_DATA_LEN];	/* data */
	...
	/* message from master */
	if (pRec->msg.canId == CO_SLEEP_MASTER_CAN_ID)  {
		...
		if (pRec->msg.data[0] == CO_SLEEP_CMD_QUERY)  {
			...
			if (error != 0u)  {
				...
				trData[0] = CO_SLEEP_STATE_OBJECTION;
				trData[1] = error;
#ifdef CO_SLEEP_454
				(void)icoTransmitMessage(sleepSlaveCob, &trData[0], 0u);
#elif defined(CO_SLEEP_447)
				(void)icoTransmitMessage(sleepSlaveCob[pRec->spec], &trData[0], 0u);
#endif /* CO_SLEEP_447 */
			} else {
				sleepState = SLEEP_STATE_OBJECTION;
			}
		}
...
#ifdef CO_SLEEP_454
	if (pRec->msg.canId == CO_SLEEP_SLAVE_CAN_ID)  {
#endif /* CO_SLEEP_454 */
#ifdef CO_SLEEP_447
	if ((pRec->msg.canId >= CO_SLEEP_SLAVE_CAN_ID)
	 && (pRec->msg.canId < (CO_SLEEP_SLAVE_CAN_ID + CO_SLEEP_447_CNT))) {
#endif /* CO_SLEEP_447 */

		if ((sleepState == SLEEP_STATE_OBJECTION)
		 && (pRec->msg.data[0] == CO_SLEEP_STATE_OBJECTION))  {
			(void)coTimerStop(&sleepTimer);
			(void)callSleepInd(CO_SLEEP_MODE_OBJECTION,
				(UNSIGNED8)(pRec->spec + 2u));
		}
	}

#ifdef CO_SLEEP_454
	if (pRec->msg.canId == CO_SLEEP_REQ_SLEEP_CAN_ID)  {
#endif /* CO_SLEEP_454 */
#ifdef CO_SLEEP_447
	if ((pRec->msg.canId >= CO_SLEEP_REQ_SLEEP_CAN_ID)
	 && (pRec->msg.canId < (CO_SLEEP_REQ_SLEEP_CAN_ID + CO_SLEEP_447_CNT))) {
#endif /* CO_SLEEP_447 */
...
CO_NMT_MASTER_STATE_T	masterState;
	...
	if (masterState != CO_NMT_MASTER_STATE_MASTER)  {
		return;
	}
	/* send objection command */
	memset(&trData[0], 0, 8u);
	trData[0] = CO_SLEEP_CMD_QUERY;
	(void)icoTransmitMessage(sleepMasterCob, &trData[0], 0);

	/* start timeout timer */
	(void)coTimerStart(&sleepTimer, waitTime * 1000ul,
		sleepNoObjection, NULL, CO_TIMER_ATTR_ROUNDUP); /*lint !e960 */
...
(void)ptr;
	...
	sleepState = SLEEP_STATE_PREPARE;
...
CO_NMT_MASTER_STATE_T	masterState;
	...
	(void)coTimerStart(&sleepTimer, waitTime * 1000ul,
		sleepSilent, NULL, CO_TIMER_ATTR_ROUNDUP);	/*lint !e960 */
		/* Derogation MisraC2004 R.16.9 function identifier used without '&'
		 * or parenthesized parameter */

	sleepState = SLEEP_STATE_SLEEP;
}
...
(void) ptr;
	...
	if (error == 0u)  {
		...
		(void)coTimerStart(&sleepTimer, 1000000ul,
			sleepDoze, NULL, CO_TIMER_ATTR_ROUNDUP); /*lint !e960 */
		/* Derogation MisraC2004 R.16.9 function identifier used without '&'
		 * or parenthesized parameter */
	} else {
		sleepState = SLEEP_STATE_AWAKE;
	}
}
	...
	)
{
	...
	if (sleepState != SLEEP_STATE_SLEEP)  {
		return;
	}
...
UNSIGNED8	nodeId = CO_NODE_ID;
	...
	(void)coTimerStart(&awakeTimer, repeatTime * 1000ul,
		sleepWakeUpMessage, &wakeUpData, CO_TIMER_ATTR_ROUNDUP); /*lint !e960 */
		/* Derogation MisraC2004 R.16.9 function identifier used without '&'
		 * or parenthesized parameter */
...
WAKEUP_T	*pWakeUp = (WAKEUP_T *)ptr;
	...
	memset(&trData[0], 0, 8u);
	trData[0] = pWakeUp->status;
	trData[1] = pWakeUp->reason;
	(void)icoTransmitMessage(pWakeUp->sleepCob, &trData[0], 0u);
}
...
* \internal
*
* \brief icoSleepSetMasterSlave - set master state
*
* \return none
*
*/
void icoSleepSetMasterSlave(
		CO_NMT_MASTER_STATE_T mState
	)
{

	if (mState == CO_NMT_MASTER_STATE_SLAVE)  {
		(void)icoCobChangeType(sleepMasterCob, CO_COB_TYPE_RECEIVE);
		(void)icoCobChangeType(sleepSlaveCob, CO_COB_TYPE_TRANSMIT);
		(void)icoCobChangeType(sleepRequestCob, CO_COB_TYPE_TRANSMIT);
	} else {
		(void)icoCobChangeType(sleepMasterCob, CO_COB_TYPE_TRANSMIT);
		(void)icoCobChangeType(sleepSlaveCob, CO_COB_TYPE_RECEIVE);
		(void)icoCobChangeType(sleepRequestCob, CO_COB_TYPE_RECEIVE);
	}
}
#endif /* CO_SLEEP_454 */
...
UNSIGNED8	trData[CO_CAN_MAX_DATA_LEN];	/* data */
	...
	/* send objection command */
	memset(&trData[0], 0, 8u);
	trData[0] = CO_SLEEP_STATE_REQ;
# ifdef CO_SLEEP_454
	(void)icoTransmitMessage(sleepRequestCob, &trData[0], 0u);
# endif /* CO_SLEEP_454 */
# ifdef CO_SLEEP_447
	(void)icoTransmitMessage(sleepSlaveCob[0], &trData[0], 0u);
# endif /* CO_SLEEP_447 */
}
#endif /* CO_NMT_MASTER */
...
UNSIGNED16	i;
	...
	if (master != 0u)  {
		...
		sleepSlaveCob[0] = icoCobCreate(CO_COB_TYPE_TRANSMIT, CO_SERVICE_SLEEP, 0u);
	}
