...
*-------------------------------------------------------------------
* $Id: co_led.c 35552 2021-03-17 15:04:44Z boe $
*
*
*-------------------------------------------------------------------
*
...
static BOOL_T		ledTimerOn;
static CO_TIMER_T	ledTimer;
static UNSIGNED8	ledCnt;
static UNSIGNED32	saveErrorStates;
...
static UNSIGNED8	ledGreenTableCnt;
static UNSIGNED8	ledRedTableCnt;
...
UNSIGNED8	i;
	...
	/* set state on/off */
	if (on == CO_TRUE)  {
		saveErrorStates |= (1ul << (UNSIGNED32)newState);
	} else {
		saveErrorStates &= ~(1ul << (UNSIGNED32)newState);
	}

	if (saveErrorStates == 0u)  {
	...
	while (i > 0u)  {
		i--;
		if ((saveErrorStates & (1ul << (UNSIGNED32)priority[i])) != 0ul)  {
			/* state found, set led */
...
UNSIGNED32	takt;
	...
	(void)coTimerStart(&ledTimer, takt, ledTimerInd, NULL,
