...
*-------------------------------------------------------------------
* $Id: co_pdo.c 41921 2022-09-01 10:39:03Z boe $
*
*
*-------------------------------------------------------------------
*
...
# ifdef CO_EVENT_DYNAMIC_PDO_UPDATE
#  define CO_EVENT_PDO_UPDATE_CNT	CO_EVENT_DYNAMIC_PDO_UPDATE
# endif /* CO_EVENT_DYNAMIC_PDO_UPATE */
...
static CO_INLINE CO_REC_PDO_T *getReceivePdo(UNSIGNED16 idx);
...
# ifdef CO_EVENT_PDO
static void pdoRecInd(UNSIGNED16 pdoNr, UNSIGNED8 transType);
static void pdoRecTimerInd(UNSIGNED16 pdoNr);
# endif /*  CO_EVENT_PDO */
#endif /* CO_PDO_RECEIVE_CNT */
#ifdef CO_PDO_TRANSMIT_CNT
static CO_INLINE CO_TR_PDO_T *getTransmitPdo(UNSIGNED16 idx);
...
# ifdef CO_EVENT_PDO_UPDATE_CNT
static CO_EVENT_PDO_UPDATE_T pdoEventUpdateTable[CO_EVENT_PDO_UPDATE_CNT];
static UNSIGNED16	pdoEventUpdateTableCnt = 0u;
# endif /* CO_EVENT_PDO_UPDATE_CNT */
#endif /* CO_PDO_TRANSMIT_CNT */
...
CO_TR_PDO_T *pPdo;
	...
	/* reload the timer if timer active */
	if ((pPdo->eventTime != 0u) && (pPdo->transType > 253u))  {
		if (coTimerIsActive(&pPdo->pdoTimer) == CO_TRUE)  {
			(void)coTimerStart(&pPdo->pdoTimer,
    			(UNSIGNED32)pPdo->eventTime * 1000ul,
    			pdoEventTimer, pPdo, CO_TIMER_ATTR_ROUNDDOWN_CYCLIC); /*lint !e960 */
...
#endif /* CO_MPDO_PRODUCER */
		{
			...
			for (m = 0u; m < mapCnt; m++)  {
				if (pPdo->mapTableConst->mapEntry[m].val == mapEntry)  {
					retVal = coPdoReqNr(pPdo->pdoNr, flags);
				}
			}
		}
	}
...
#endif /* CO_MPDO_PRODUCER */
	/* if there is no valid mapping count, don't transmit it */
	if (pPdo->mapTableConst->mapCnt == 0u)  {
		return(RET_MAP_LEN_ERROR);
	}

	memset(&trData[0], 0, sizeof(trData));
...
#ifdef CO_EVENT_PDO_UPDATE_CNT
		for (utCnt = 0u; utCnt < pdoEventUpdateTableCnt; utCnt++)  {
			index = (UNSIGNED16)(pPdo->mapTableConst->mapEntry[cnt].val >> 16u);
			subIndex = (UNSIGNED8)((pPdo->mapTableConst->mapEntry[cnt].val >> 8u) & 0xffu);
			pdoEventUpdateTable[utCnt](index, subIndex);
		}
#endif /* CO_EVENT_PDO_UPDATE_CNT */

#ifdef CO_DYNAMIC_OBJDIC
# ifdef CO_DYNAMIC_OD_PTR_REFRESH
		if (pPdo->mapTable != NULL)  {
			/* update address again for get/set OD */
			pPdo->mapTable->mapEntry[cnt].pVar = 
				coOdGetObjAddr(
				pPdo->mapTableConst->mapEntry[cnt].val >> 16,
				(pPdo->mapTableConst->mapEntry[cnt].val >> 8) & 0xff);
			}
# endif /* CO_DYNAMIC_OD_PTR_REFRESH */
		...
		if (((offs % 8u) == 0u)
		 && ((pPdo->mapTableConst->mapEntry[cnt].len % 8u) == 0u))  {
			...
			CO_OS_LOCK_OD
			/* numeric value ? */
			coNumMemcpyUnpack(&trData[offs >> 3],
			...
			if (pPdo->mapTableConst->mapEntry[cnt].len == 1u)  {
				...
				trData[(offs >> 3)] |= (tmpBuf[0] & 0x1u) << (offs & 7u);
			} else {
				/* move bytevar to can buffer */
				for (i = 0u; i < byteLen; i++)  {
					/* low part */
					trData[(offs >> 3) + i] |= (UNSIGNED8)(tmpBuf[i] << (offs & 7u));
					/* high part */
					trData[(offs >> 3) + i + 1u] =
						tmpBuf[i] >> (8u - (offs & 7u));
				}
...
CO_TR_PDO_T	*pPdo;
	...
	pPdo = getTransmitPdo(pRecData->spec);
...
CO_REC_PDO_T	*pPdo;
	...
	pPdo = getReceivePdo(pRecData->spec);
...
*/
	...
	if (pPdo->eventTime != 0u)  {
		(void) coTimerStart(&pPdo->pdoTimer,
			(UNSIGNED32)pPdo->eventTime * 1000ul,
			pdoReceiveEventTimer, pPdo, CO_TIMER_ATTR_ROUNDDOWN); /*lint !e960*/
		/* Derogation MisraC2004 R.16.9 function identifier used without '&'
		 * or parenthesized parameter */
	...
	pPdo->timeStamp = pRecData->msg.timestamp;
...
# ifdef CO_DYNAMIC_OD_PTR_REFRESH
			/* update address again for get/set OD */
			pRecPdo->mapTable->mapEntry[cnt].pVar = 
				coOdGetObjAddr(
				pRecPdo->mapTableConst->mapEntry[cnt].val >> 16,
				(pRecPdo->mapTableConst->mapEntry[cnt].val >> 8) & 0xff);
# endif /* CO_DYNAMIC_OD_PTR_REFRESH */
#endif /* CO_DYNAMIC_OBJDIC */
			/* lock OD */
			CO_OS_LOCK_OD
			...
			if (changed == CO_TRUE)  {
				...
				(void)icoEventObjectChanged(pDesc, index, subIndex,
					changed);
			}
...
# ifdef CO_EVENT_PDO
	/* user indication */
	pdoRecInd(pRecPdo->pdoNr, pRecPdo->transType);
# endif /*  CO_EVENT_PDO */
...
/***************************************************************************/
/**
* \brief coPdoGetTimestamp - get PDO timestamp
*
* This function returns the timestamp of the PDO
* set by the CAN driver
*
* \return 
*
*/
CO_CAN_TIMESTAMP_T coPdoGetTimestamp(
		UNSIGNED16		pdoNr		/**< pdo number */
	)
{
CO_REC_PDO_T	*pPdo;
    ...
    pPdo = searchRecPdo(pdoNr);
	if (pPdo == NULL)  {
		return((CO_CAN_TIMESTAMP_T) 0);
	}
	return(pPdo->timeStamp);
}
# endif /* CO_CAN_TIMESTAMP_SUPPORTED */
		...
		if ((pTrPdo->cobId & CO_COB_VALID_MASK) != CO_COB_INVALID)  {
			...
			if (pTrPdo->transType <= 240u)  {
				...
				if (pTrPdo->transType == 0u)  {
					...
					/* cyclic PDO */
					if ((syncCounterVal >= pTrPdo->syncStartVal)
					 || (syncCounterVal == 0u))  {
						pTrPdo->actSyncCnt--;
...
CO_TR_PDO_T *pPdo;
	...
	if ((canId & CO_COB_VALID_MASK) == 0u)  {
		...
		if (icoCheckRestrictedCobs(canId, 0x180u, 0x180u) == CO_TRUE)
#else /* CO_PROFILE_447 */
...
CO_TR_PDO_T *pPdo;
	...
	retVal = icoOdGetObjTrMapData(
			...
			(UNSIGNED8)(value & 0xffu),
			&pVar,
			&len,
			&numeric);

	/* only map error or idx not found for SDO allowed */
	if ((retVal != RET_OK) && (retVal != RET_IDX_NOT_FOUND))  {
...
CO_TR_PDO_T *pPdo;
	...
	switch (subIndex)  {
		...
		case 2u:
			...
			retVal = setupTrPdoTimer(pPdo);
			/* setup inhibit time again
			 * CiA 301 doesnt require to disable cobid before changing the transmission type...
			 */
			/* set inhibit - only for transtype fe and ff (otherwise 0) */
			if (pPdo->transType >= 254u)  {
				retVal = icoCobSetInhibit(pPdo->cob, pPdo->inhibit);
			} else {
				retVal = icoCobSetInhibit(pPdo->cob, 0u);
			}
			break;
...
CO_REC_PDO_T *pPdo;
	...
	/* check mapping entry */
	retVal = icoOdGetObjRecMapData(mapIdx, mapSubIdx,
			(UNSIGNED8)(mapEntry & 0xffu), &pVar, &len, &numeric);
	if (retVal != RET_OK)  {
		/* only map error or idx not found for SDO allowed */
...
* \internal
*
* \brief pdoRecInd - check user indication
*
*/
...
* \internal
*
* \brief pdoRecTimerInd - receive event occured
*
* This function is called, if receive event timer was timed out
*
*
*/
static void pdoRecTimerInd(
		UNSIGNED16		pdoNr		/* pdo number */
	)
{
#  ifdef CO_EVENT_PDO_CNT
...
/***************************************************************************/
/**
* \brief coEventRegister_PDO_UPDATE - register PDO Data Update event
*
* Register an indication function for PDO Data Update.
*
* Before a PDO is transmitted, this function is called.
* Application can now update the given objects at the OD
* before the data are transmitted with a PDO.
*
* \return RET_T
*
*/

RET_T coEventRegister_PDO_UPDATE(
		CO_EVENT_PDO_UPDATE_T pFunction	/**< pointer to function */
    )
{
	if (pdoEventUpdateTableCnt >= CO_EVENT_PDO_UPDATE_CNT) {
		return(RET_EVENT_NO_RESSOURCE);
	}

	/* set new indication function as first at the list */
	pdoEventUpdateTable[pdoEventUpdateTableCnt] = pFunction;
	pdoEventUpdateTableCnt++;
...
* \internal
*
* \brief icoPdoSearchTransmitPdo - search for pdo transmit index
*
*
* \return index
*
*/
CO_INLINE CO_TR_PDO_T *icoPdoSearchTransmitPdo(
		UNSIGNED16		pdoNr		/* pdo number */
	)
{
UNSIGNED16	cnt;
...
* \internal
*
* \brief getTransmitPdo - get pdo Transmit and return pointer to PDO
*
*
* \return PDO pointer
*
*/
static CO_INLINE CO_TR_PDO_T *getTransmitPdo(
		UNSIGNED16		idx
	)
{
	if (idx > pdoTrCnt)  {
...
* \internal
*
* \brief getReceivePdo - get pdo receive and return pointer to PDO
*
*
* \return PDO pointer
*
*/
static CO_INLINE CO_REC_PDO_T *getReceivePdo(
		UNSIGNED16		idx
	)
{
	if (idx > pdoRecCnt)  {
...
# ifdef CO_TR_PDO_DYN_MAP_ENTRIES
		...
		pPdo->mapTableConst = pPdo->mapTable;
