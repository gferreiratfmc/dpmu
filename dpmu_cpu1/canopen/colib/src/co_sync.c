...
*-------------------------------------------------------------------
* $Id: co_sync.c 30776 2020-02-20 07:51:40Z boe $
*
*
*-------------------------------------------------------------------
*
...
(void)ptr;
	...
	/* transmit data */
	(void) icoTransmitMessage(syncCob, &trData[0], MSG_OVERWRITE);

	if (counterValue != 0u)  {
		icoSyncHandler(trData[0]);
	} else {
		icoSyncHandler(0u);
	}
			...
			if ((cyclePeriod != 0u)
			 && ((syncCobId & CO_SYNC_PRODUCER_BIT) != 0u))  {
				...
				retVal = coTimerStart(&syncTimer,
					cyclePeriod,
					syncEventTimer, NULL, CO_TIMER_ATTR_ROUNDDOWN_CYCLIC); /*lint !e960 */
