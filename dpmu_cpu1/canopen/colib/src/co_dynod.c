...
*-------------------------------------------------------------------
* $Id: co_dynod.c 31801 2020-04-23 14:27:41Z boe $
*
*
*-------------------------------------------------------------------
*/
...
/* There are a lot of array handlings here.
 * Therfore disable misra check for that.
 * Otherwise, we had to add an example for a lot of lines 
 */
/*lint -esym(960, 17.4)*/
    ...
    )
{
	...
	dynData.pOdAssign =
		(CO_OD_ASSIGN_T *)malloc(objCnt * sizeof(CO_OD_ASSIGN_T));/*lint !e586*/
	/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	dynData.pOdDesc = (CO_OBJECT_DESC_T*)malloc(dynData.maxDescCnt * sizeof(CO_OBJECT_DESC_T)); /*lint !e586*/
	/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (u8Cnt != 0u)  {
		/* get memory */
		dynData.pU8 =
			(UNSIGNED8 **)malloc(u8Cnt * sizeof(UNSIGNED8 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (u16Cnt != 0u)  {
		/* get memory */
		dynData.pU16 =
			(UNSIGNED16 **)malloc(u16Cnt * sizeof(UNSIGNED16 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (u32Cnt != 0u)  {
		/* get memory */
		dynData.pU32 =
			(UNSIGNED32 **)malloc(u32Cnt * sizeof(UNSIGNED32 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (i8Cnt != 0u)  {
		/* get memory */
		dynData.pI8 =
				(INTEGER8 **)malloc(i8Cnt * sizeof(INTEGER8 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (i16Cnt != 0u)  {
		/* get memory */
		dynData.pI16 =
			(INTEGER16 **)malloc(i16Cnt * sizeof(INTEGER16 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (i32Cnt != 0u)  {
		/* get memory */
		dynData.pI32 =
			(INTEGER32 **)malloc(i32Cnt * sizeof(INTEGER32 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	if (u64Cnt != 0u)  {
		/* get memory */
		dynData.pU64 =
			(UNSIGNED64 **)malloc(u64Cnt * sizeof(UNSIGNED64 *)); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	...
	)
{
	...
	if (dynData.pOdAssign != NULL)  {
		free(dynData.pOdAssign); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'free' is deprecated */
	}

	/* release memory for subindex description */
	if (dynData.pOdDesc != NULL)  {
		free(dynData.pOdDesc); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}

	/* release memory */
	if (dynData.pU8 != NULL)  {
		free(dynData.pU8); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}
	if (dynData.pU16 != NULL)  {
		free(dynData.pU16); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}
	if (dynData.pU32 != NULL)  {
		free(dynData.pU32); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}
	if (dynData.pI8 != NULL)  {
		free(dynData.pI8); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}
	if (dynData.pI16 != NULL)  {
		free(dynData.pI16); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}
	if (dynData.pI32 != NULL)  {
		free(dynData.pI32); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}

#ifdef CO_EXTENDED_DATA_TYPES
	if (dynData.pU64 != NULL)  {
		free(dynData.pU64); /*lint !e586*/
		/* Derogation MisraC2004 R.20.4 function 'malloc' is deprecated */
	}
#endif /* CO_EXTENDED_DATA_TYPES */
...
RET_T retVal;
	...
	switch (pDesc->dType)  {
	case CO_DTYPE_U8_PTR:
		dynData.pU8[pDesc->tableIdx] = (UNSIGNED8 *) pVar;
		break;
	case CO_DTYPE_U16_PTR:
		dynData.pU16[pDesc->tableIdx] = (UNSIGNED16 *) pVar;
		break;
	case CO_DTYPE_U32_PTR:
		dynData.pU32[pDesc->tableIdx] = (UNSIGNED32 *) pVar;
		break;
	case CO_DTYPE_I8_PTR:
		dynData.pI8[pDesc->tableIdx] = (INTEGER8 *) pVar;
		break;
	case CO_DTYPE_I16_PTR:
		dynData.pI16[pDesc->tableIdx] = (INTEGER16 *) pVar;
		break;
	case CO_DTYPE_I32_PTR:
		dynData.pI32[pDesc->tableIdx] = (INTEGER32 *) pVar;
		break;
	default:
		retVal = RET_DATA_TYPE_MISMATCH;
		break;
...
/**
*
* \brief coDynOdGetObjDescPtr - get dynod obj description pointer
*
* Get description pointer for a dynamic object
*
* \return RET_T
*
*/
RET_T coDynOdGetObjDescPtr(
		UNSIGNED16 index,			/**< index */
		UNSIGNED8 subIndex,			/**< subindex */
		CO_CONST CO_OBJECT_DESC_T **pDescPtr	/**< pointer desc structure*/
	)
{
CO_DYN_DATA_T *pDyn = &dynData;
CO_OBJECT_DESC_T *pDesc;
...
/**
*
* \brief coDynOdGetObjAddr - return dynod object adresse
*
* get dynamic object address
*
* \return RET_T
*
*/
UNSIGNED8 *coDynOdGetObjAddr(
...
void *pObjAddr = NULL;
	...
	switch (pDesc->dType)  {
		case CO_DTYPE_U8_PTR:
			if (dynData.pU8 != NULL)  {
				pObjAddr = dynData.pU8[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_U16_PTR:
			if (dynData.pU16 != NULL)  {
				pObjAddr = dynData.pU16[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_U32_PTR:
			if (dynData.pU32 != NULL)  {
				pObjAddr = dynData.pU32[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_I8_PTR:
			if (dynData.pI8 != NULL)  {
				pObjAddr = dynData.pI8[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_I16_PTR:
			if (dynData.pI16 != NULL)  {
				pObjAddr = dynData.pI16[pDesc->tableIdx];
			}
			break;
		case CO_DTYPE_I32_PTR:
			if (dynData.pI32 != NULL)  {
				pObjAddr = dynData.pI32[pDesc->tableIdx];
			}
			break;
#ifdef CO_EXTENDED_DATA_TYPES
		case CO_DTYPE_U64_PTR:
			if (dynData.pU64 != NULL)  {
				pObjAddr = dynData.pU64[pDesc->tableIdx];
			}
			break;
#endif /* CO_EXTENDED_DATA_TYPES */
		default:
...
/**
*
* \brief coDynOdGetObjSize - return size for dynamic object
*
* Get size of given object
*
* \return RET_T
*
*/

UNSIGNED32 coDynOdGetObjSize(
		CO_CONST CO_OBJECT_DESC_T	*pDesc	/**< pointer for description index */
	)
{
UNSIGNED32	size = 0u;
