...
*-------------------------------------------------------------------
* $Id: co_p401.c 38501 2021-10-14 09:26:00Z boe $
*
*
*-------------------------------------------------------------------
*
...
#define CO_401_DIGOUT_MAPPED		1UL
#define CO_401_ANOUT_MAPPED			2UL
...
int port;
	...
	while (mappingTable[count] != 0) {
		if (pdo == (mappingTable[count] & 0xFFu)) {
	...
	)
{
	...
	if ((newState == CO_NMT_STATE_STOPPED) && (execute == CO_TRUE)) {
				...
				if (globalInterrupt != 0u) {
					...
					if (((currentValue ^ newValue) & tmp) != 0u) {
						...
						if ((((currentValue & newValue) ^ newValue) & tmp)  != 0u) {
	                        ...
	                        Serial_debug(DEBUG_NONE, &cli_serial, "401: detected Low->High change at digin port %d\r\n", port);
							intFlag = 1;
						}
						...
						if ((((currentValue ^ newValue) & ~newValue) & tmp)  != 0u) {
	                        ...
	                        Serial_debug(DEBUG_NONE, &cli_serial, "401: detected High-Low change at digin port %d\r\n", port);
							intFlag = 1;
						}
					}
...
REAL32	floatValue, prescaler, offset;
RET_T retval;
	...
	if (retval == RET_OK) {
		...
		for (port = 1u; port <= maxport; port++) {
			...
			if (retval == RET_OK) {
				 ...
				 * Spec V 3.1 (June/2013) excludes scaling
				 * ===========================================
				 */
				...
				/* get pre-scaling value */
				retval = coOdGetObj_r32(0x642fu + CO_401_PROFILE_OFFSET,
					port, &prescaler);
				if (retval == RET_OK)  {
					/* get offset value */
					retval = coOdGetObj_r32(0x642eu + CO_401_PROFILE_OFFSET,
						port, &offset);
					if (retval == RET_OK)  {
						/* calculate analog input prescaled float 0x6403 */
						floatValue = (REAL32)newValue * prescaler + offset;
						(void)coOdPutObj_r32(0x6403u + CO_401_PROFILE_OFFSET,
							port, floatValue);
					}
				}
