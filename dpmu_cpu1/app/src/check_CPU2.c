...
static bool check_SoH(void)
{
    ...
    /* update SoH
     * only if new charge have been done
     * */
    if(IPC_isFlagBusyRtoL(IPC_CPU1_L_CPU2_R,
                          IPC_NEW_CONSTANT_CURRENT_CHARGE_DONE))
    {
        ret = calculate_SoH();
        if(RET_OK == ret)
        ...
        /* moved to inside calculate_SoH()
         * kept here as a reminder that it is needed and used
         * */
//        IPC_ackFlagRtoL(IPC_CPU1_L_CPU2_R,
//                        IPC_NEW_CONSTANT_CURRENT_CHARGE_DONE);
    }
