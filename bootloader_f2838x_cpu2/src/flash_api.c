...
FlashRet_t flash_api_read(uint32_t address, uint32_t* readBuffer, uint32_t len){
    FlashRet_t status;
    Fapi_StatusType fret;
    ...
    EALLOW;
    fret = Fapi_doMarginRead((uint32_t*)address, readBuffer, len, Fapi_NormalRead);
